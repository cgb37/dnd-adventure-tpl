...
- [x] Add config-driven gating (allowlist + denylist)
- [x] Inject global chatbot shell into the shared layout
- [x] Implement split view (desktop/tablet) + bottom drawer (mobile)
- [x] Persist open/provider/kind state via `localStorage`
- [x] Stabilize UI tests by building to `_site-ui/` (avoid `jekyll serve` contention)
- [x] Add/adjust Playwright coverage for presence/absence + persistence

Reference plan: `.github/plans/issue-3-iteration-4-global-chatbot.plan.md`

## Security considerations

- Browser rendering is XSS-hardened by using `textContent` (not `innerHTML`) for model output.
- The global widget does not store secrets; it only calls the API at `site.llm_api_base_url`.
- API auth remains server-owned (Iteration 4 reuses the Iteration 3 security model).
- Promotion/generation are server-side file operations; the browser only submits prompts and triggers promote by kind/slug.
- LocalStorage persistence is non-sensitive state only (open/collapsed, provider, kind).
...
sequenceDiagram
  ...
  Browser->>Jekyll: GET DM page (chapter/location/etc.)
  Jekyll-->>Browser: HTML + chatbot shell + JS/CSS
  Browser->>API: GET /v1/meta/providers
  Browser->>API: GET /v1/meta/generators
  Browser->>API: POST /v1/generate/{kind}
  API-->>Browser: { draft_path, slug, ... }
  Browser->>API: POST /v1/promote/{kind}/{slug} (optional)
  API-->>Browser: { to: promoted_path }
```
...
```diff
- docs/iteration-4.md
+ docs/issue-3-iteration-4-global-chatbot.md
+
+ _includes/chatbot_shell.html
+ assets/css/chatbot.css
+ assets/js/chatbot-widget.js
+ tests/ui/global-chatbot.spec.cjs
+ .github/plans/issue-3-iteration-4-global-chatbot.plan.md
+
~ _config.yml
~ _layouts/default.html
~ _includes/styles.html
