...
async def generate_encounter(*, request: GenerateRequest, campaign: str) -> GeneratedDraft:
    settings = Settings()
    ...
    agent = build_agent(output_type=EncounterOutput, system_prompt=SYSTEM_PROMPT)
    try:
    ...
    out: EncounterOutput = result.output
